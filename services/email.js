...
export const sendVerifyEmailRequest = async ({ firstName, email, token }) => {
    try {
        await transporter.sendMail({
            ...
            html: `Hello ${firstName},<br/><br/>The link below will have you set up your account, this link expires in one day after this message has been sent.<br />Please note, you must complete this step in order to login to Moving Forward. Contact us if this link expires.<br/><br/><a href="${BASE_URL}/verify/${token}/${firstName}/0">Go set up my account with Moving Forward.</a><br/><br/> Best regards,<br/>Moving Forward, LLC.`
        });
    }
    catch (err) {
        console.error(err);
